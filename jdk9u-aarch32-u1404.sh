...
# boot jdk
$D wget -nv https://github.com/ojdkbuild/contrib_jdk9u-ci/releases/download/jdk-9.0.4%2B12/jdk-9.0.4-ojdkbuild-linux-x64.zip
...
# sources
$D wget -nv http://hg.openjdk.java.net/jdk-updates/jdk9u/archive/${OJDK_TAG}.tar.bz2
...
for repo in `echo corba hotspot jaxp jaxws jdk langtools nashorn` ; do
    $D wget -nv http://hg.openjdk.java.net/jdk-updates/jdk9u/${repo}/archive/${OJDK_TAG}.tar.bz2
    $D tar -xjf ${OJDK_TAG}.tar.bz2
    $D rm ${OJDK_TAG}.tar.bz2
    $D mv ${repo}-${OJDK_TAG} ./jdk9u/${repo}
done
